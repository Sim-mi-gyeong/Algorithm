    ...
    for i in range(1, m + 1):
        ...
        for j in range(4):
            ...
            if 0 <= nx < n and 0 <= ny < n:
                ...
                if info_graph[nx][ny][1] == 0 and graph[nx][ny] == 0:
                    print("info_graph 상태 : ", info_graph)
        ...
        ### 이거 다시 - 자신의 냄새가 있는 방향으로 이동
        ### 이때 우선순위에 따른 방향을 먼저 찾아보기
        # tmp_next_move_list = []
        for j in range(4):
            nextDir = sharkDir[i - 1][currDir - 1][j]
            nx = currX + dx[nextDir]
            ny = currY + dy[nextDir]
            if 0 <= nx < n and 0 <= ny < n:
                if info_graph[nx][ny][0] == i:  # 자신이 뿌린 향수라면
                    # 그 방향으로 방향 틀기
                    # 이동 후 상어의 위치 기록
                    print("자신의 향수가 있는 방향 : ", nextDir)
                    print("이동 전 상어가 있는가? : ", graph[nx][ny])
                    graph[nx][ny] = i
                    print("이동 후 상어가 있는가? : ", graph[nx][ny])
                    # 이동 전 상어의 위치는 빈칸 처리
                    graph[currX][currY] = 0
                    shark_loc_dic[i] = (nx, ny)
                    # 이동 후 상어의 이동 방향 기록
                    shark_dir_dic[i] = nextDir
                    # 자신이 뿌린 향수가 아직 남아있는 위치로 돌아온 경우, 향수를 또 뿌리지는 않음
                    break
